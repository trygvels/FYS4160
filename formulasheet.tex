...
\subsection{Rotating (Kerr) Black holes}
Stationary but not static. Two killing vectors $K=\partial_t$ and $R=\partial_{\phi}$. Also possesses a Killing tensor. Two horizons: In the ergosphere you can move radially, but forced in $\phi$-direction.

\begin{align}
    ds^2 &= -\left(1-\frac{2GMr}{\rho^2}\right)dt^2 -\frac{2GMar\sin^2\theta}{\rho^2}(dtd\phi+d\phi dt) \nonumber \\
    + &\frac{\rho^2}{\delta}dr^2 +\rho^2d\theta^2+\frac{\sin^2\theta}{\rho^2}\left[(r^2+a^2)^2-a^2\Delta \sin^2\theta\right]d\phi^2 \\
...
How to check for event horizons \\
KILLING TENSORS? \\
\end{multicols}
\end{document}
